...
function LaptopInfo({ soleTrending }) {
  const urlUpdated = window.location.href;

  return (
    <>
      <Helmet>
        <title>{soleTrending.name}</title>
      ...
      <section className="flex flex-wrap p-4 mb-4 mt-2">
        <div className="w-full md:w-1/3">
          <img src={soleTrending.image} alt="Laptop" className="w-full rounded" />
        </div>
        ...
        <div className="w-full md:w-2/3 md:pl-5">
          ...
          <div className="mb-4 border-b border-gray-300 pb-2 text-sm text-gray-600">
            Brand:{' '}
            <Link
              to={`/laptop-deals/${soleTrending.brand}`}
      ...
      <section>
        <h3 className="border-b border-gray-300 font-semibold text-lg pb-3 mb-5">Product Details</h3>
        <p className="text-gray-800 text-sm leading-relaxed">{soleTrending.details}</p>
      </section>
    </>
  );
}
